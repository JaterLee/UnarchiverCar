...
+ (void)exportCarFileAtPath:(NSString *)carFilePath toExportPath:(NSString *)exportPath {
    ...
    if ([exportPath containsString:header]) {
        exportPath = [exportPath stringByReplacingOccurrencesOfString:header withString:@""];
    }
    exportCarFileAtPath(carFilePath, exportPath);
}
